...
<head>
    ...
    <title>Guide Me Forum | Edit Profile</title>
    <!-- ///////////////////////////////////////////// -->
    <meta charset="utf-8">
...
<div class="box">
	<div class="box_left">
    	<a href="<?php echo $url_home; ?>">Forum Index</a> &gt; Edit Profile
    </div>
	<div class="box_right">
		...
		if($_POST['password']==$_POST['passverif'])
		{
			if(strlen($_POST['password'])>=6)
			{
				if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
				{
					...
					$email = ($_POST['email']);
					$dn = $con->query('select count(*) as nb from users where username="'.$username.'"')->fetch_assoc();
					if($dn['nb']==0 or $_POST['username']==$_SESSION['username'])
					{
						if($con->query('update users set username="'.$username.'", password="'.$password.'", email="'.$email.'" where id="'.($_SESSION['userid']).'"'))
						{
							$form = false;
...
<?php
	...
	if($form)
	{
		...
		{
			$dnn = $con->query('select username,email from users where username="'.$_SESSION['username'].'"')->fetch_assoc();
...
?>
    <form action="edit_profile.php" method="post">
        <h4 style="margin-bottom: 1%">Edit Profile</h4><br />
        <div class="center">
